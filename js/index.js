...
function resizeCanvas() {
  ...
  var radius = (gridWidth / columns) / 2; //원의 반지름
  var padding = radius //원 사이의 간격

  placeCircles(rows, columns, radius, padding);
...
function adjustCircleSize(row, col, rows, cols, scrollPosition) {
  if (isPartOfM(row, col, rows, cols) == 0) {
    // 'M'의 일부인 경우 크기를 늘림
    return Math.min(10, 5 + scrollPosition / 100); // 예시 크기 계산
  } else {
    // 'M'이 아닌 경우 크기를 줄임
    return Math.max(2, 5 - scrollPosition / 100); // 예시 크기 계산
  }
}
...
function placeCircles(rows, columns, radius, padding) { //row가 가로 길쭉, col은 세로 길쭉

  var gridHeight = rows * (2 * radius + padding) - padding; // 전체 그리드의 높이

  // 페이지 중앙에서 시작되도록 시작 X, Y 좌표 조정
  var startX = (canvas.width - columns * (2*radius + padding) + padding) / 2;
  var startY = (canvas.height - gridHeight) / 2;

  for (var row = 0; row < rows; row++) {
    for (var col = 0; col < columns; col++) {
      var x = startX + col * (2 * radius + padding);
      var y = startY + row * (2 * radius + padding);
      radius = adjustCircleSize(row, col, rows, columns, scrollPosition);
      drawCircle(x, y, radius, 'none'); // 원의 색상은 파란색으로 설정
    }
  }
}
